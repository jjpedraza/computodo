...
function categorias_cuantos($id){
require("config.php");
    $sql = "SELECT * FROM misproductos WHERE categoria='".$id."'";
    //$sql = "SELECT count(*) FROM misproductos WHERE categoria='".$id."' ";
...
function categoriassub_cuantos($cat, $sub){
require("config.php");
    $sql = "SELECT * FROM misproductos WHERE (categoria='".$cat."' AND subcategoria='".$sub."')";
    //$sql = "SELECT count(*) FROM misproductos WHERE categoria='".$id."' ";
...
function costo($id,$costo_usd)
{
	
require("config.php");
    $sql = "SELECT * FROM misproductos WHERE id='$id'";
    $rc= $conexion -> query($sql);
    
    if($f = $rc -> fetch_array())
        {
            if (($f['usd']=='FALSE') or ($f['usd']==''))
                {
                    return $f['costo'];
                }
                else 
...
function ponerfoto($archivo,$a)
{
if (file_exists($archivo)){
	if ($a == TRUE)
		{return '<a href="'.$archivo.'" title="Haga clic aqui para descargarlo" target="new"><img src="'.$archivo.'"></a>';}
	else
		{return '<img src="'.$archivo.'">';}
	
	}
else {
	return '<img src="img/fotonodisponible.png">';}
...
function subir($nombredelcontrol, $archivo, $donde,  $ext)
{
$msgE='';	
	...
	if (substr($_FILES[$nombredelcontrol]['type'], 0, 11)=="application"){
		...
		else
		{
			if ($_FILES[$nombredelcontrol]['size']<2000000) {
				$target_path = "".$donde."/";
				$target_path = $target_path .$archivo.'.'.$ext; 
				if(move_uploaded_file($_FILES[$nombredelcontrol]['tmp_name'], $target_path)) 
					{ $msgE= "El archivo ". $archivo.'.'.$ext. " ha sido subido con exito";
					} else{
					 $msgE= "Ha ocurrido un error con ".$nombredelcontrol.", trate de nuevo!";
					}
			} else {
...
function nombredearchivo($variable){
	$variable2 = str_replace(' ', '', $variable); // elimina espacios dentro de la variable
	...
	$variable2 = str_replace(';', '', $variable2); 
	$variable2 = str_replace(':', '', $variable2); 
	$variable2 = str_replace('Ñ', 'N', $variable2); 
	$variable2 = str_replace('ñ', 'n', $variable2); 
	$variable2 = str_replace('-', '', $variable2); 
	$variable2 = str_replace(',', '', $variable2); 
	$variable2 =strtolower($variable2); // todo a minusculas
